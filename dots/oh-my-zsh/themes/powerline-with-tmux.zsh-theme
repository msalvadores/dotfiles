...
function tmux_git {
    if [ -n "$TMUX" ]; then
        tmux set -g status-left-length 60 > /dev/null
        GIT_PROMPT=$(git_prompt_info)
        tmux set-window-option status-left "#[fg=colour16,bg=colour254,bold] #S | #(basename `pwd`) $GIT_PROMPT #[fg=colour254,bg=colour234,nobold]" > /dev/null

        POWERLINE_WHITE="%F{blue}%K{white}"$'\ue0b0'"%F{white}%F{black}%K{white}%F{white}"
PROMPT="$POWERLINE_SEC1_BG$POWERLINE_SEC1_TXT%k%f$POWERLINE_SEC1_FG%K{blue}"$'\ue0b0'"%k%f%F{white}%K{blue}%F{blue}"$POWERLINE_WHITE"%k"$'\ue0b0'"%f "

    PROMPT="
"$PROMPT
    fi
}
add-zsh-hook precmd tmux_git
